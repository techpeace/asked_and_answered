...
module ApplicationHelper
  ...
  def byline(question)
    byline = ''
    byline += question.child_name if question.child_name
    byline += " from " + question.child_city if question.child_city
    byline += ", " + question.child_state if question.child_state
    byline
  end
  ...
  def action_links(object, options = {})
    ...
    capture_haml do
      haml_tag :td, :class => "list-table-links" do
        haml_concat link_to(icon(options[:edit_icon]), 
              {:action => "edit", 
               :controller => class_name, 
               :id => object.id}, 
              :class => "edit-link")
        haml_concat button_to_remote('', 
